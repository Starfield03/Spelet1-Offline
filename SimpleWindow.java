 ...
 * Tillagt square, poly och circle 2015-10-08
 */
public class SimpleWindow {
	...
	protected JFrame frame; // the frame
	protected SWCanvas canvas; // the drawing area
	...
	/**
	 * Moves the pen to a new position  drawing a circle with radius width.
	 * 
	 * @param x
	 *            the x coordinate of the new position
	 * @param y
	...
	class WindowEventHandler extends WindowAdapter {
		public void windowClosing(WindowEvent e) {
         ...
         /** Circle tillagt till ursprunbgsklassen Simple Window
     * @param x
     * @param y
     * @param width
         */
    	public void circle(int x, int y, int width) {
		canvas.circle(x, y, width);
	}
         /** square tillagt till ursprunbgsklassen Simple Window
     * @param x
     * @param y
     * @param width
         */
        public void square(int x, int y, int width){
           canvas.square(x, y, width);
        }
         /** square tillagt till ursprunbgsklassen Simple Window
     * @param x[]
     * @param y[]
     * @param npoints
        ...
        public void polygon(int x[], int y[], int npoints){
            canvas.polygon(x, y, npoints);
        }
                
        public String inText( String prompt){
            JFrame framen = new JFrame("x");
            String name = JOptionPane.showInputDialog(framen, prompt);
            return name;
        }
        public int inInt( String prompt){
            ...
            while(true){
                try{
                    svar=Integer.parseInt(name);
                    break;
                }catch (NumberFormatException e){
                   System.out.println("Fel indata");
                   name = JOptionPane.showInputDialog(framen, prompt);
                }
            }
...
class SWCanvas extends JPanel {
	...
	void circle(int x, int y, int width) {
		Graphics2D g = (Graphics2D) img.getGraphics();
